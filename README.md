...
- **Results overlay (optional):** when `LIVE_FIXTURES_URL` is set, the pipeline overlays **finished** match scores from the live feed (via the [Worker](worker/README.md) `/fixtures` endpoint) on top of the upbound base. Falls back to upbound alone if unavailable.
...
A small [Cloudflare Worker](worker/README.md) proxies the free, no-key community
API at [worldcup26.ir](https://worldcup26.ir) to power two things: **in-match
scores** in the schedule banner/cards (`/live`) and a finished-results overlay in
the data pipeline (`/fixtures`). The Worker serves both endpoints from one cached
upstream fetch refreshed every ~90s (gentle on the upstream). See
[`worker/README.md`](worker/README.md) to deploy it. Note: no reputable *free* API
offers WC 2026 real-time scores, so live in-match data here is best-effort.
